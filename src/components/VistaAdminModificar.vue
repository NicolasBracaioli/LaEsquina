<template>
  <div class="admin-page">
    ...
    <div class="card">
      ...
      <form class="form" @submit.prevent="modificarProducto">
...
const modificarProducto = async () => {
  ...
  try {
    ...
    mensaje.value = resp.data?.message || "Producto actualizado correctamente";
  } catch (e: any) {
    exito.value = false;
    mensaje.value =
      e?.response?.data?.message || "Error al actualizar producto";
    console.error("Modificar producto:", e?.response?.data || e.message);
  } finally {
    loading.value = false;
  }
};
...
.card {
  background: #ffffff;
  width: 600px;             /* 👈 igual que Insertar: más grande */
  margin: 40px auto 0;
